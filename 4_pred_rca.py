if __name__ == '__main__':
    ...
    predictor.initialize_from_trained_model_folder(
        join(nnUNet_results, 'Dataset002_RCA/nnUNetTrainer__nnUNetPlans__2d'),
        use_folds=('all'),
        checkpoint_name='checkpoint_final.pth',
    )
    # variant 1: give input and output folders
    predictor.predict_from_files(join(nnUNet_raw, 'Dataset002_RCA/imagesTs'),
                                join(nnUNet_raw, 'Dataset002_RCA/imagesTs_pred'),
    ...
    rca_res_path = os.path.join(nnUNet_raw, 'Dataset002_RCA/imagesTs_pred')
